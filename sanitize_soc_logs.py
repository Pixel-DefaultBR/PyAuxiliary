...
SOC Log Sanitizer
=================
Script para remover e substituir informações sensíveis de logs de SOC.
...
class SOCLogSanitizer:
    """Sanitiza informações sensíveis em logs de SOC."""
    ...
    def __init__(self,
        ...
        Args:
            use_hash: Se True, usa hash SHA256 para substituições (mantém consistência)
            preserve_structure: Se True, preserva estrutura (ex: email@domain -> REDACTED@REDACTED)
            custom_replacements: Dicionário personalizado de substituições por tipo
        """
        self.use_hash = use_hash
        self.preserve_structure = preserve_structure
        self.custom_replacements = custom_replacements or {}

        # Cache de substituições para manter consistência
    ...
    def sanitize_email(self, text: str) -> str:
        """Remove/substitui endereços de email."""
    ...
    def sanitize_ipv4(self, text: str) -> str:
        """Remove/substitui endereços IPv4."""
    ...
    def sanitize_ipv6(self, text: str) -> str:
        """Remove/substitui endereços IPv6."""
    ...
    def sanitize_domain(self, text: str) -> str:
        """
        Remove/substitui domínios (exceto aqueles em emails já substituídos).
        """
        # Primeiro sanitiza emails para não substituir domínios dentro deles
        pattern = re.compile(self.PATTERNS['domain'])

        def replace_domain(match):
            domain = match.group(0)
            # Ignora se for domínio comum/genérico (lista pode ser expandida)
    ...
    def sanitize_username(self, text: str) -> str:
        """Remove/substitui nomes de usuários."""
    ...
    def sanitize_client_name(self, text: str) -> str:
        """Remove/substitui nomes de clientes."""
    ...
    def sanitize_all(self, text: str) -> str:
        ...
        Args:
            text: Texto do log a ser sanitizado

        Returns:
            Texto sanitizado
        """
        # Ordem importa: sanitiza emails antes de domínios para evitar duplicação
    ...
    def sanitize_file(self, input_path: str, output_path: Optional[str] = None) -> None:
        """
        Sanitiza um arquivo de log completo.

        Args:
            input_path: Caminho do arquivo de entrada
            output_path: Caminho do arquivo de saída (se None, adiciona '.sanitized')
        """
    ...
    def get_statistics(self) -> Dict[str, int]:
        """Retorna estatísticas de quantos itens foram sanitizados."""
...
def sanitize_text(text: str, use_hash: bool = False) -> str:
    """
    Função auxiliar para sanitizar texto rapidamente.

    Args:
        text: Texto a ser sanitizado
        use_hash: Se True, usa hash para substituições
...
def sanitize_log_file(input_path: str, output_path: Optional[str] = None, use_hash: bool = False) -> None:
    """
    Função auxiliar para sanitizar arquivo de log.
...
if __name__ == "__main__":
    ...
    # Exemplo 1: Sanitização com contadores
    ...
    # Exemplo 2: Sanitização com hash
    print("\n" + "=" * 70)
    ...
    # Exemplo 3: Substituições customizadas
    print("\n" + "=" * 70)
    print("\n✨ LOG SANITIZADO (substituições customizadas):")
    print("-" * 70)
    custom = {
